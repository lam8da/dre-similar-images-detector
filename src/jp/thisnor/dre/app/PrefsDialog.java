...
class PrefsDialog extends Dialog {
  private static final String DRE_CORE_PACKAGE_PATH = "jp.thisnor.dre.core"; //$NON-NLS-1$
  ...
  private void createContents() {
    ...
    prefsTableViewer
        .getTable()
        .setLayoutData(new FormDataBuilder().left(0).right(100).top(0).bottom(closeButton).build());
    closeButton.setLayoutData(
        new FormDataBuilder().width(120).right(100).height(30).bottom(100).build());
  }

  private final IContentProvider PREFS_TABLE_CONTENT_PROVIDER =
  ...
  private final CellLabelProvider PREFS_TABLE_KEY_LABEL_PROVIDER =
  ...
  private final CellLabelProvider PREFS_TABLE_VALUE_LABEL_PROVIDER =
      new CellLabelProvider() {
        @Override
        public void update(ViewerCell cell) {
          String key = (String) cell.getElement();
  ...
  private final MouseListener PREFS_TABLE_CELL_CLICKED_LISTENER =
      new MouseAdapter() {
        @Override
  ...
  private final ViewerSorter PREFS_TABLE_VIEWER_SORTER =
      new ViewerSorter() {
        @Override
  ...
  private final SelectionListener CLOSE_BUTTON_SELECTION_LISTENER =
