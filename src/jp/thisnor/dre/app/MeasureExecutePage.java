...
class MeasureExecutePage extends DREPage {
  ...
  private ProgressBar loadProgress, measureProgress;
  ...
  void createContents(Composite parent) {
    ...
    loadProgress.setLayoutData(
        new FormDataBuilder().left(loadProgressLabel).right(100).top(0).build());
    measureProgressLabel.setLayoutData(
        new FormDataBuilder().left(0).right(0, 160).top(loadProgressLabel).build());
    measureProgress.setLayoutData(
        new FormDataBuilder().left(measureProgressLabel).right(100).top(loadProgressLabel).build());
            ...
            try {
              ...
              Display.getDefault()
                  .asyncExec(
                      new Runnable() {
                        public void run() {
                          if (!frame.getShell().isDisposed()) frame.setNextButtonEnabled(true);
                        }
                      });
      ...
      msgBox.setText(
          messages.getString(
              executor.isTerminated()
                  ? "MeasureExecutePage.CONFIRM_BACK_MESSAGE"
                  : "MeasureExecutePage.CONFIRM_ABORT_TITLE"));
