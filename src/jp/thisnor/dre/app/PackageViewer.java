...
class PackageViewer {
  ...
  void createContents(Composite parent) {
    ...
    descText =
        new Text(descTextComp, SWT.MULTI | SWT.READ_ONLY | SWT.WRAP | SWT.BORDER | SWT.V_SCROLL);
    ...
    descText.setLayoutData(
        new FormDataBuilder().left(0).right(100).top(infoLabel).bottom(0, 100).build());

    packageIcon.setLayoutData(
        new FormDataBuilder().left(0).right(0, 32).top(0).bottom(0, 32).build());
    descTextComp.setLayoutData(
        new FormDataBuilder().left(packageIcon).right(100).top(0).bottom(100).build());
    ...
    optionTableViewer
        ...
        .getTable()
        .setLayoutData(
            new FormDataBuilder().left(0).right(100).top(optionTableLabel).bottom(100).build());

    descComp.setLayoutData(new FormDataBuilder().left(0).right(100).top(0).build());
    optionTableComp.setLayoutData(
        new FormDataBuilder().left(0).right(100).top(descComp).bottom(100).build());
  }
  ...
  void setActivePackage(MeasurerPackage handlerPackage) {
    if (handlerPackage == null) {
      ...
      nameLabel.setText(
          handlerPackage.getName() != null
              ? handlerPackage.getName()
              : "(Untitled handler)"); //$NON-NLS-1$
      infoLabel.setText(
          (handlerPackage.getVersion() != null ? "version " + handlerPackage.getVersion() : "")
              + //$NON-NLS-1$ //$NON-NLS-2$
              ", "
              + //$NON-NLS-1$
              (handlerPackage.getAuthor() != null
                  ? handlerPackage.getAuthor()
                  : "(Unknown author)")); //$NON-NLS-1$
      descText.setText(
          handlerPackage.getDescription() != null
              ? handlerPackage.getDescription()
              : ""); //$NON-NLS-1$
      packageIcon.setImage(
          handlerPackage.getImage() != null ? handlerPackage.getImage() : defaultPackageIcon);
