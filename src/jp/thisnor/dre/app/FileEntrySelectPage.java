...
public class FileEntrySelectPage extends DREPage {
  ...
  private static final String DEFAULT_EXTENSION_FILTER = ".*\\.(?i:png|jpe?g|gif|bmp)";
  ...
  void hiddened() {
    ...
    prefs.setValue(
          PREFS_TARGET_FILES_KEY, fileEntryListToText(targetFileListViewer.getExtPathList()));
    if (prefs.getBoolean(PREFS_SOURCE_FILES_KEY + ".save")) //$NON-NLS-1$
    prefs.setValue(
          PREFS_SOURCE_FILES_KEY, fileEntryListToText(storageFileListViewer.getExtPathList()));
  }
  ...
  private String fileEntryListToText(List<String> filePathList) {
    ...
    for (String filePath : filePathList) {
      ...
      else sb.append(File.pathSeparatorChar);
      sb.append(filePath);
    }
    return sb.toString();
  }
