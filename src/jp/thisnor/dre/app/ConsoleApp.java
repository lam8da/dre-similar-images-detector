...
class ConsoleApp {
  ...
  private static final ProgressListener STDERR_PROGRESS_LISTENER =
  ...
  boolean run() {
    ...
    if (optionStrMap != null) {
      for (Entry<String, String> e : optionStrMap.entrySet()) {
        ...
        if (candidates != null && !candidates.contains(e.getValue())) {
          System.err.println(
              "WARNING: Specified option value is not allowed: " + e.getKey() + "=" + e.getValue());
          continue;
        }
    ...
    WholeTask task =
        new WholeTask(
            targetPathList,
            storagePathList == null ? targetPathList : storagePathList,
            filter,
            measurerPackage,
            numThreads,
  ...
  private void printInXML(Map<String, Long> fidMap, List<SimilarGroup> simGroupList) {
    ...
    for (Entry<String, Long> entry : fidMap.entrySet()) {
      System.out.printf(
          "  <file id=\"%d\" path=\"%s\" />%n",
          entry.getValue(), new File(entry.getKey()).getAbsolutePath());
    }
    for (SimilarGroup simGroup : simGroupList) {
      System.out.printf(
          "  <simgroup file=\"%d\">%n", fidMap.get(simGroup.getFileEntry().getPath()));
      for (SimilarEntry sim : simGroup.getSimilarList()) {
        System.out.printf(
            "    <simitem distance=\"%d\" file=\"%d\" />%n",
            sim.getDistance(), fidMap.get(sim.getFileEntry().getPath()));
      }
