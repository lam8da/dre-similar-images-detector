...
public class PackageSelectPage extends DREPage {
  ...
  private static final String PREFS_PACKAGE_KEY =
      "jp.thisnor.dre.core.PackageSelectPage.selectedpackage"; //$NON-NLS-1$
  private static final String PREFS_NUM_THREAD_KEY =
      "jp.thisnor.dre.core.PackageSelectPage.numthread"; //$NON-NLS-1$
  ...
  void createContents(Composite parent) {
    ...
    for (File packageFile : PACKAGE_DIR.listFiles()) {
      try {
        packageList.add(
            MeasurerPackage.importPackage(packageFile, frame.getMessages().getLocale()));
    ...
    packageViewerGroup.setLayoutData(
        new FormDataBuilder().left(0).right(100).top(0).bottom(systemOptionComp).build());
  ...
  private final SelectionListener PACKAGE_LIST_SELECTION_LISTENER =
