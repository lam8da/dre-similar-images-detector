...
class FileDropListViewer {
  ...
  private Image
      ...
      imageFileIconImage,
      noFileIconImage,
      emptyFolderIconImage; // folderIconImage
  ...
  FileDropListViewer(DREFrame frame, Composite parentComp) {
    ...
    DropTarget dropTarget =
        new DropTarget(fileTable, DND.DROP_MOVE | DND.DROP_COPY | DND.DROP_LINK);
    ...
    for (String exPath : exPaths) {
      ...
      for (int i = 0; i < fileTable.getItemCount() - 2; i++) {
        ...
        if (fe != null && fe.getPath().equals(toRawPath(exPath))) {
          preSub =
              item.getData(TABLE_EDITOR_KEY) != null
                  ? ((Button) ((TableEditor) item.getData(TABLE_EDITOR_KEY)).getEditor())
                      .getSelection()
                  : false;
      ...
      if (preSub) {
        ...
        if (editorCtrl != null) ((Button) editorCtrl.getEditor()).setSelection(preSub);
      }
      if (preProps != null) {
  ...
  private final SelectionListener FILE_TABLE_SELECTION_LISTENER =
        ...
        public void widgetSelected(SelectionEvent event) {
          ...
          if (item == appendDirItem) {
            DirectoryDialog dialog =
                new DirectoryDialog(Display.getDefault().getActiveShell(), SWT.MULTI);
            ...
            dialog.open();
            if (dialog.getFileNames() != null) addFiles(dialog.getFileNames());
          }
        }
      };

  private final KeyListener FILE_TABLE_KEY_LISTENER =
      new KeyAdapter() {
        @Override
  ...
  private final ControlListener FILE_TABLE_RESIZE_LISTENER =
      new ControlAdapter() {
        @Override
  ...
  private final PaintListener FILE_TABLE_PAINT_LISTENER =
      new PaintListener() {
        @Override
        public void paintControl(PaintEvent event) {
          for (int i = 0; i < fileTable.getItemCount() - 2; i++) {
            TableItem item = fileTable.getItem(i);
            TableEditor editorCtrl = (TableEditor) item.getData(TABLE_EDITOR_KEY);
            if (editorCtrl != null) editorCtrl.getEditor().redraw();
          }
        }
      };

  private final Listener FILE_TABLE_MEASURE_ITEM_LISTENER =
      new Listener() {
  ...
  private final Listener FILE_TABLE_ERASE_ITEM_LISTENER =
      new Listener() {
        public void handleEvent(Event event) {
  ...
  private final Listener FILE_TABLE_PAINT_ITEM_LISTENER =
      new Listener() {
        public void handleEvent(Event event) {
          ...
          gc.drawText(
              props != null
                  ? props
                  : messages.getString("FileDropListViewer.REPORT_CALCULATING_MESSAGE"),
              event.x + 48,
              event.y + 20,
              true);
        }
  ...
  private final DropTargetListener FILE_TABLE_DROP_LISTENER =
      new DropTargetAdapter() {
        ...
        public void drop(DropTargetEvent event) {
          int insertPos =
              event.item != null && event.item.getData() != null
  ...
  private final DragSourceListener FILE_TABLE_DRAG_LISTENER =
  ...
  private final SelectionListener FILE_TABLE_CHANGE_TO_DIR_LISTENER =
      new SelectionAdapter() {
        @Override
        public void widgetSelected(SelectionEvent event) {
  ...
  private final SelectionListener FILE_TABLE_CHANGE_TO_FILE_LISTENER =
      new SelectionAdapter() {
        @Override
        public void widgetSelected(SelectionEvent event) {
          TableItem item = fileTable.getSelection()[0];
  ...
  private final SelectionListener FILE_TABLE_REMOVE_ITEM_LISTENER =
      new SelectionAdapter() {
        @Override
        public void widgetSelected(SelectionEvent event) {
          removeItems(fileTable.getSelectionIndices());
  ...
  private final SelectionListener FILE_TABLE_CHECKBOX_LISTENER =
      new SelectionAdapter() {
        @Override
        public void widgetSelected(SelectionEvent event) {
  ...
  private class ComputeFilePropertiesTask implements Runnable {
    ...
    private ComputeFilePropertiesTask(TableItem item) {
      ...
      this.recursive =
          item.getData(TABLE_EDITOR_KEY) != null
              ? ((Button) ((TableEditor) item.getData(TABLE_EDITOR_KEY)).getEditor()).getSelection()
              : false;
    }
    ...
    private String computeDirProperties() {
      ...
      String text =
          String.format(
              ...
              dirProps.numDirs,
              dirProps.numFiles,
              getFileSizeDescription(dirProps.length));
      return text;
    }
  ...
  private final Runnable FILE_TABLE_REDRAW_TASK =
      new Runnable() {
        public void run() {
  ...
  private String getFileSizeDescription(long byteSize) {
    ...
    return String.format(
        "%d.%d%s", viewByteSize / 10, viewByteSize % 10, FILE_SIZE_UNIT[curUnitId]); //$NON-NLS-1$
  }
}
