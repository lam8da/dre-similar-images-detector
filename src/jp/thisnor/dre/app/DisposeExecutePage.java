...
class DisposeExecutePage extends DREPage {
  ...
  void createContents(Composite parent) {
    ...
    disposeProgress.setLayoutData(
        new FormDataBuilder().left(disposeProgressLabel).right(100).top(0).build());
  ...
  void activated() {
    ...
    disposeProgressLabel.setText(
        messages.getString(
            "DisposeExecutePage.DISPOSE_WAY_TEXT." + (disposeWay == 0 ? "DELETE" : "MOVE")));
    ...
    switch (disposeWay) {
      ...
      case 1:
        if (!dirMoveTo.exists()) {
          MessageBox msgBox =
              new MessageBox(Display.getDefault().getActiveShell(), SWT.YES | SWT.NO);
          ...
          if (!dirMoveTo.mkdirs()) {
            logText.setText(
                String.format(
                    messages.getString("DisposeExecutePage.REPORT_FAILED_MKDIR_MESSAGE"),
                    dirMoveTo.getPath()));
            executor.shutdownNow();
            return;
          }
  ...
  private void log(final String text) {
    Display.getDefault()
        .asyncExec(
            new Runnable() {
              public void run() {
                if (!logText.isDisposed()) logText.append(text + "\n"); //$NON-NLS-1$
              }
            });
  }
    ...
    public void run() {
      if (file instanceof NormalFileEntry) {
        ...
        if (!f.delete()) {
          log(
              String.format(
                  messages.getString("DisposeExecutePage.REPORT_FAILED_DELETE_MESSAGE"),
                  file.getPath()));
        }
      }
    ...
    public void run() {
      if (file instanceof NormalFileEntry) {
        ...
        if (!f.renameTo(newF)) {
          log(
              String.format(
                  messages.getString("DisposeExecutePage.REPORT_FAILED_MOVE_MESSAGE"),
                  file.getPath()));
        }
      }
      disposeLatch.countDown();
